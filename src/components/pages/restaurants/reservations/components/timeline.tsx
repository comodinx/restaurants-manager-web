...
export function Timeline(props: TimelineProperties) {
  ...
  const formattedDate = moment(date, constants.dates.formatReservationDate)
    .format(constants.dates.formatReservationDateForHummands);
  const tables: any = restaurant.tables.reduce((acc: any, table: any) => {
    acc[table.id] = table;
    return acc;
  }, {});
  const entries = timeline.filter((entry: any) => tables[entry.tableId].capacity >= selectedNumGuests);
      ...
      <li className="mb-16 ms-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 mt-4 border border-restaurants-200 rounded-lg shadow-lg">
          {entries.map((entry: any, i: any) => (
            <Table key={i} date={date} table={tables[entry.tableId]} available={entry.available} onSelectTable={onSelectTable} />
          ))}
        </div>
      </li>
    </>
